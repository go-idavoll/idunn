...
| `TestUnconfirmedUpdateIsRolledBackAndNotOfferedAgain` | IDN-39: the app updates itself from 1.0.0 to 1.1.0 with `--probation-attempts 2`. Two launcher starts never confirm; the third rolls back to 1.0.0, prints why, and removes the 1.1.0 tree while the journal still says 1.1.0 committed. The app's own self-update then reports 1.1.0 as rolled back and installs nothing (exit 3), and hands the rollback to its Reporter exactly once across two checks (`rolled_back`, phase `probation`, class `unconfirmed`). 1.2.0 is offered, confirmed with `--mark healthy`, and stays through further starts. |
| `TestUnhealthyUpdateIsRolledBackAtTheNextStart` | IDN-39: the updated app reports `--mark unhealthy` with a reason; the next launcher start rolls back to 1.0.0 and prints the application's reason. |
| `TestReleasePolicyDecidesTheProbation` | IDN-39: 1.1.0 is published with `probation: { attempts: 1 }` in `pack.yaml`; the app updates with `--probation-follow-release --probation-attempts 2` and is rolled back after one start. 1.2.0 published with `attempts: 0` is not put on probation. |
