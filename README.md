...
| Delta stage 2 (binary patches) | patch format implemented on both sides (reader in `core/stage`, generator in `internal/delta`, standard library only); patch targets are not published or fetched yet |
