...
// The same, on the wire: the decoder on the caller's side drops an unknown class,
// so asserting on the decoded error alone would not notice a helper that put the
...
// socketDir is a short, private directory for a helper socket.
//
// Short because the kernel bounds a socket path (104 bytes on macOS) and
// t.TempDir() under /var/folders is longer than that; under /tmp because it is
// root-owned and sticky on every POSIX system, which is what checkSocketDir
// accepts above the socket's own directory.
...
// A socket path the kernel would refuse with EINVAL is refused by name instead.
func TestAnEndpointTooLongForTheKernelIsRefused(t *testing.T) {
	_, err := elevate.NewHelper(elevate.HelperOptions{
...
// Being in the allow-list is not enough: a root an ordinary user controls is
// refused when the helper starts, before a socket exists (IDN-22).
func TestAnAllowedRootSomeoneElseControlsStopsTheHelperFromStarting(t *testing.T) {
	if os.Geteuid() == 0 {
	...
	_, err := elevate.NewHelper(elevate.HelperOptions{
		Endpoint:     endpoint,
		Applier:      &recorder{},
		AllowedRoots: []string{filepath.Join(t.TempDir(), "acme")},
	})
	if !errors.Is(err, elevate.ErrUnsafeRoot) {
		t.Fatalf("VULNERABILITY: err = %v, want ErrUnsafeRoot", err)
	}
...
// The socket's directory must not be one another user can move aside: a
// group- or world-writable parent without the sticky bit lets them rename it and
// put their own in its place.
func TestASocketDirectoryUnderAWritableParentIsRefused(t *testing.T) {
	parent := socketDir(t)
...
// blocker is an Applier that waits until released, so a test can observe what
// the caller does while an apply is in progress.
type blocker struct {
	started  chan struct{}
	release  chan struct{}
...
// Cancelling the caller stops its wait, not the privileged apply: the helper owns
// the apply once it has read the request, and an unprivileged process that could
// abort it mid-swap would be the half-installed state the journal prevents.
func TestCancellingTheCallerDoesNotStopTheApply(t *testing.T) {
