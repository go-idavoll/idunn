...
| §5 | Installer flow | **done** — `core/installer` plus the `cmd/installer` binary: embedded anchor, flags, elevation decision, exit codes, and a default install root per platform and `--scope user\|machine` (`installer.DefaultRoot`, IDN-24). `cmd/launcher --uninstall` removes an installation offline and crash-safe, never following a link, and then itself (`core/uninstall`, IDN-35) |
