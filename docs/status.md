...
| §6.4 | Delta stage 2 (binary patches) | **partial** — the client side is complete: the format on both sides (`stage.ApplyPatch`, `internal/delta`), the walk a skipped-releases client follows (`release.Chain`, `trust.Versions`), and staging that reconstructs a changed file from what is on disk plus the cheapest published patches, verifying each hop and falling back to the full target whenever it cannot. The packer does not emit patch targets yet, so nothing is published to patch with (IDN-14) |
