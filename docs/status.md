...
| §5 | Installer flow | **done** — `core/installer` plus the `cmd/installer` binary: embedded anchor, flags, elevation decision, exit codes, and a default install root per platform and `--scope user\|machine` (`installer.DefaultRoot`, IDN-24). `cmd/launcher --uninstall` removes an installation offline and crash-safe, never following a link, and then itself (`core/uninstall`, IDN-35). OS integrations are recorded in `.updater/integrations.json` and removed from that record (`core/integrate`, IDN-36): the Windows "Installed apps" entry is registered by `cmd/installer`, kept at the live version after every `Apply` and at every launcher start, and removed by `--uninstall`, never touching an entry of another root; repair and the other integrations (shortcuts, associations, protocols, autostart) are open |
| §6.1 | Blue/green layout + pointer | **done** — `internal/layout`, symlink (POSIX) / pointer file (Windows), plus the launcher shim (`core/launch`, `cmd/launcher`), which also restarts an application that asks for it (exit code 42 / `launch.Relaunch`, IDN-29); on Windows the launcher and the application share a kill-on-close job that is released after a normal exit, and console control events are left to the application (IDN-40). A committed update can be put on probation: the launcher counts starts, `launch.MarkHealthy`/`MarkUnhealthy` report the outcome, and a version that does not confirm is rolled back crash-safely outside the journal and blocked for the updater (a host policy, or each release's signed policy published from `pack.yaml`; a rollback is reported through the `Reporter` at the next check; an update over an unconfirmed version returns to the last confirmed one, which GC keeps, IDN-39) |
...
| §14.5 | Telemetry + staged rollout | **done** — `Reporter` with a closed error-class vocabulary, including failed probations the launcher leaves for the updater (IDN-39); local rollout bucketing |
